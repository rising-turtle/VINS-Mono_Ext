                          ...
                          Eigen::Matrix2d& J) const
{
    ...
    double dxdmx = 1.0 + rad_dist_u + k1 * 2.0 * mx2_u + k2 * rho2_u * 4.0 * mx2_u + k3 * rho2_u * 6.0 * mx2_u * rho2_u + 2.0 * p1 * p_u(1) + 6.0 * p2 * p_u(0);
    double dydmx = k1 * 2.0 * p_u(0) * p_u(1) + k2 * 4.0 * rho2_u * p_u(0) * p_u(1) + k3 * 6.0 * rho2_u * rho2_u * p_u(0) * p_u(1) + p1 * 2.0 * p_u(0) + 2.0 * p2 * p_u(1);
    double dxdmy = dydmx;
    double dydmy = 1.0 + rad_dist_u + k1 * 2.0 * my2_u + k2 * rho2_u * 4.0 * my2_u + k3 * rho2_u * 6.0 * my2_u * rho2_u + 6.0 * p1 * p_u(1) + 2.0 * p2 * p_u(0);
...
PinholeCamera::readParameters(const std::vector<double>& parameterVec)
{
    ...
    params.fx() = parameterVec.at(5);
    params.fy() = parameterVec.at(6);
    params.cx() = parameterVec.at(7);
    params.cy() = parameterVec.at(8);
...
PinholeCamera::writeParameters(std::vector<double>& parameterVec) const
{
    ...
    parameterVec.at(4) = mParameters.k3();
    ...
    parameterVec.at(5) = mParameters.fx();
    parameterVec.at(6) = mParameters.fy();
    parameterVec.at(7) = mParameters.cx();
    parameterVec.at(8) = mParameters.cy();
