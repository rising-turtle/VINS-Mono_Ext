  ...
  methods
    function obj = KalmanFilter(varargin)
      if nargin == 2 && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        my_ptr = varargin{2};
        gtsam_wrapper(796, my_ptr);
      elseif nargin == 1 && isa(varargin{1},'numeric')
        my_ptr = gtsam_wrapper(797, varargin{1});
      else
    ...
    function delete(obj)
      gtsam_wrapper(798, obj.ptr_gtsamKalmanFilter);
    end
    ...
    function varargout = predict(this, varargin)
      ...
      % 
      % Method Overloads
      % predict(GaussianDensity p, Matrix F, Matrix B, Vector u, Diagonal modelQ)
    ...
    function varargout = predict2(this, varargin)
      ...
      % 
      % Method Overloads
      % predict2(GaussianDensity p, Matrix A0, Matrix A1, Vector b, Diagonal model)
      if length(varargin) == 5 && isa(varargin{1},'gtsam.GaussianDensity') && isa(varargin{2},'double') && isa(varargin{3},'double') && isa(varargin{4},'double') && isa(varargin{5},'gtsam.noiseModel.Diagonal')
    ...
    function varargout = predictQ(this, varargin)
      ...
      % 
      % Method Overloads
      % predictQ(GaussianDensity p, Matrix F, Matrix B, Vector u, Matrix Q)
    ...
    function varargout = update(this, varargin)
      ...
      % 
      % Method Overloads
      % update(GaussianDensity p, Matrix H, Vector z, Diagonal model)
    ...
    function varargout = updateQ(this, varargin)
      ...
      % 
      % Method Overloads
      % updateQ(GaussianDensity p, Matrix H, Vector z, Matrix Q)
