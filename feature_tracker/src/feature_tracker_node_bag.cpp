/*
    July 28 2019, He Zhang, hzhang8@vcu.edu 
    
    Read bag file and publish imu to vins_estimator 
...
{
    ros::init(argc, argv, "feature_tracker_ext");
    ros::NodeHandle n("~");
    ...
    */
    ...
    // set up topics to load 
    std::vector<std::string> topics;
    string imu_topic_name = "/imu0"; 
    n.param("imu_topic", imu_topic_name, imu_topic_name); 
    string img_topic_name = "/cam0/color";
    n.param("img_topic", img_topic_name, img_topic_name); 
    topics.push_back(imu_topic_name); 
    topics.push_back(img_topic_name); 

    // read rosbag 
    rosbag::View view(bagIn, rosbag::TopicQuery(topics));
    ros::Publisher imu_msg_pub = n.advertise<sensor_msgs::Imu>("imu0", 10); 
	...
	if(it->getTopic() == imu_topic_name)
	{
	    ...
	    imu_msg_pub.publish(*(imuMsg.get()));
	}else if(it->getTopic() == img_topic_name)
	{
	    sensor_msgs::Image::ConstPtr imgMsg = it->instantiate<sensor_msgs::Image>(); 
	    img_callback(imgMsg); 
	}else{
	    ROS_WARN("feature_tracker_node_bag.cpp: unexpected msg: %s", it->getTopic().c_str()); 
	}
	ros::spinOnce(); 
    }
