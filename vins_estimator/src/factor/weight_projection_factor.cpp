#include "weight_projection_factor.h"
#include <iostream>

using namespace std; 

Eigen::Matrix2d WeightProjectionFactor::sqrt_info;
...
bool WeightProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    TicToc tic_toc;
...
void WeightProjectionFactor::check(double **parameters)
{
    ...
    for (int k = 0; k < 19; k++)
    {
    	...
    	tmp_residual = (pts_camera_j / dep_j).head<2>() - pts_j.head<2>();
#endif
        tmp_residual = w_* sqrt_info * tmp_residual;
